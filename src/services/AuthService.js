...
const { RpcProvider, typedData, hash, num, constants } = require('starknet');
...
class AuthService {
     ...
     * REGISTER USER
     */
    async register(name, email, pass, company, identity, refId = '') {
        ...
        if (existingUser) {
            return { status: false, error: 'email_not_available' };
        }
     ...
     * Input: Encrypted String (contains { key: user_id, code: random_code })
     */
    async verifyEmailProcess(encryptedString) {
        try {
            
            const decryptor = new EncryptionService(encryptedString);
            const jsonStr = decryptor.decrypt(); 

            if (!jsonStr) {
                return { status: false, error: 'security_violation' };
            }
            ...
            try {
                ...
                return { status: false, error: 'malformed_verification_data' };
            }
            ...
            if (typeof payload.key !== 'string' || typeof payload.code !== 'string') {
                return { status: false, error: 'malformed_verification_data' };
            }

            const now = this.date.timestampTimeNow();
            
            const verifyRecord = await this.Security.findOne({ 
                user_id: payload.key,
                code: payload.code,
                type: 'account_opening',
                status: 0,
            ...
            if (!verifyRecord) {
                return { status: false, error: 'verification_link_expired_or_used' };
            }
            ...
            return { status: false, error: 'internal_processing_error' };
        }
    }
    ...
    async onboarding(userId, business, dob, gender, country, purpose) {
        try {
           
            if(business.dun_number) {
                 //Check if this business dun has been registered by another user
                const businessRecord = await this.User.findOne({ 'business.dun_number': business.dun_number });
                if(businessRecord && businessRecord.user_id !== userId) {
                    return { status: false, error: "business_already_registered" };
                }

                //Check if this business has already been registered for this user
                const existingBusiness = await this.User.findOne({ user_id: userId, 'business.dun_number': business.dun_number });
                if(existingBusiness) {
                    return { status: false, error: "business_already_registered" };
                }
                ...
                business.created_at = this.date.timestampTimeNow();
                business.serial_number = crypto.randomBytes(32).toString('hex');
                business.business_id = this.enc.uuid();

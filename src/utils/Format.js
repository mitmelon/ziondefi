...
function formatMoneyInt(value, decimals = 2) {
    ...
    // 1. Convert to BigInt safely without floating-point math
    if (typeof value === 'bigint') {
        ...
        if (s.includes('.')) {
            // Safely handle decimals (e.g., "9.91") by string manipulation, avoiding math rounding bugs
            const parts = s.split('.');
            let integerPart = parts[0];
            let fractionalPart = parts[1].substring(0, decimals).padEnd(decimals, '0');
            
            // Fix edge case for values passed like "-.91" or ".91"
    ...
    // 3. Format integer part (Restored YOUR original Regex - safe across all environments)

...
class DateHelper {
    ...
    getCurrentDateTimeByCountry(countryName) {
        ...
        // We pick the first timezone for the country, just like your PHP code did
    ...
    /**
     * Compare dates between two countries
     */
    canStart(nigeriaDateTime, targetCountry) {
        // 1. Parse Nigeria Time
        // Assuming nigeriaDateTime is a string 'YYYY-MM-DD HH:mm:ss'
        const ngCountry = ct.getCountry('NG');
        const ngZone = ngCountry ? ngCountry.timezones[0] : 'Africa/Lagos';
        
        // Create the date object in Nigeria time
        const ngDate = dayjs.tz(nigeriaDateTime, ngZone);

        // 2. Get Target Country Time
        ...
        // 3. Get Current Time in Target Zone
        const targetNow = dayjs().tz(targetZone);

        // 4. Compare (Is targetNow >= ngDate?)
        // Note: We must compare timestamps to be accurate across zones
    ...
    formatTimeAgo(dateInput) {
        // If input is timestamp string/number

...
mod ZionDefiFactory {
    ...
    struct Storage {
        ...
        global_settlement_delay: u64,
        merchant_settlement_delay: Map<ContractAddress, u64>,
        merchant_has_custom_delay: Map<ContractAddress, bool>,
        merchant_instant_settlement: Map<ContractAddress, bool>,
    ...
    enum Event {
        ...
        SettlementDelayUpdated: SettlementDelayUpdated,
    ...
    #[derive(Drop, starknet::Event)]
    struct SettlementDelayUpdated { key: felt252, merchant: ContractAddress, delay: u64, timestamp: u64 }
    #[derive(Drop, starknet::Event)]
    ...
    fn constructor(
        ...
        // Default settlement delay: 30 minutes
        self.global_settlement_delay.write(DEFAULT_SETTLEMENT_DELAY);
    }
    ...
    impl ZionDefiFactoryImpl of ziondefi::interfaces::IZionDefiFactory<ContractState> {
        ...
        fn create_card(
            ...
            let (card_address, _) = deploy_syscall(
                self.vault_class_hash.read(), 0, calldata.span(), false,
        ...
        // ================================================================
        // B. PROTOCOL CONFIGURATION (owner-only)
        // ================================================================
        ...
        // ================================================================
        // C. RELAYER MANAGEMENT (owner-only)
        // ================================================================
        ...
        // ================================================================
        // D. TOKEN MANAGEMENT (relayer-only)
        // ================================================================
        ...
        // ================================================================
        // E. SETTLEMENT CONFIGURATION (relayer-only)
        // ================================================================

        fn set_global_settlement_delay(ref self: ContractState, delay_seconds: u64) {
            self._assert_relayer();
            self.global_settlement_delay.write(delay_seconds);
            self.emit(SettlementDelayUpdated { key: 'global', merchant: Zero::zero(), delay: delay_seconds, timestamp: get_block_timestamp() });
        }

        fn set_merchant_settlement_delay(ref self: ContractState, merchant: ContractAddress, delay_seconds: u64) {
            self._assert_relayer();
            assert(self.merchant_registered.entry(merchant).read(), 'Not registered');
            self.merchant_settlement_delay.entry(merchant).write(delay_seconds);
            self.merchant_has_custom_delay.entry(merchant).write(true);
            self.emit(SettlementDelayUpdated { key: 'merchant', merchant, delay: delay_seconds, timestamp: get_block_timestamp() });
        }

        fn set_merchant_instant_settlement(ref self: ContractState, merchant: ContractAddress, instant: bool) {
            self._assert_relayer();
            assert(self.merchant_registered.entry(merchant).read(), 'Not registered');
            self.merchant_instant_settlement.entry(merchant).write(instant);
            self.emit(SettlementDelayUpdated {
                key: if instant { 'instant_on' } else { 'instant_off' },
                merchant, delay: 0, timestamp: get_block_timestamp(),
            });
        }

        // ================================================================
        // F. MERCHANT REGISTRY (relayer-only)
        // ================================================================
        ...
        fn get_effective_settlement_delay(self: @ContractState, merchant: ContractAddress) -> u64 {
            if self.merchant_has_custom_delay.entry(merchant).read() {
                self.merchant_settlement_delay.entry(merchant).read()
            } else {
                self.global_settlement_delay.read()
            }
        }

        fn is_merchant_instant_settlement(self: @ContractState, merchant: ContractAddress) -> bool {
            self.merchant_instant_settlement.entry(merchant).read()
        }
    }
    ...
    impl InternalImpl of InternalTrait {
        /// Only the authorized relayer may call this function.
        fn _assert_relayer(self: @ContractState) {
            let caller = get_caller_address();
            assert(caller == self.authorized_relayer.read(), 'Relayer only');
        }
    }
}
